		...
		<div class="container">
			...
			<table class="table table-bordered">
				...
				<tbody>
					...
					<tr>
						<td><a href="#" data-toggle="modal" data-target="#detailPost${post.id}">${post.title}</a></td>
						<td>${post.description}</td>
						<td>${post.user.name}</td>
						<td>${post.createdAt}</td>
						<td><a href="editPost?id=${post.id}">Edit</a></td>
						<td><a href="#" data-toggle="modal" data-target="#confirmModal${post.id}">Delete</a></td>
					</tr>
					</c:forEach>
				</tbody>
			</table>
				  ...
				  <div class="modal-dialog modal-dialog-centered" role="document">
				    <div class="modal-content">
				      ...
				      <div class="modal-footer bg-danger">
				        <button type="button" class="btn btn-primary" onclick="deletePost(${post.id})">Yes</button>
			...
			<c:forEach var="post" items="${postLists}" varStatus="looop">
				<div class="modal fade" id="detailPost${post.id}" tabindex="-1" role="dialog" aria-labelledby="postTitle" aria-hidden="true">
				  <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable" role="document">
				    <div class="modal-content">
				     ...
				     <div class="modal-body">
				     	<p class="small font-weight-bold text-muted">Created by <span class="text-success">${post.user.name}</span> at <span class="text-primary">${post.createdAt}</span></p>
				     	<p class="small font-weight-bold text-muted">Updated by <span class="text-success">${post.user2.name}</span> at <span class="text-primary">${post.updatedAt}</span></p>
