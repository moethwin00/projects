...
public class PasswordController {
    ...
    public ModelAndView changePassword(@Validated @ModelAttribute(value = "password") PasswordForm passwordForm,
        ...
        if (result.hasErrors() || !passwordForm.getNewPassword().equals(passwordForm.getConfirmNewPassword())
                || !BCrypt.checkpw(passwordForm.getOldPassword(), password)) {
            ...
            if (!BCrypt.checkpw(passwordForm.getOldPassword(), password)) {
                model.addObject("invalidPassword", messageSource.getMessage(CommonConstant.MSG_0007, null, null));
            }
            model.setViewName("changepassword");
            return model;
        } else {
