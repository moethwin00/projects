...
public class PostServiceImpl implements PostService {
    ...
    public PostCreateForm setDataToPostCreateForm(Post post, HttpServletRequest request) {
        ...
        if (StringUtil.isNotEmpty(request.getParameter("postTitle"))) {
        ...
        while (line != null) {
            ...
            if (attributes != null) {
                if (attributes.length == 3) {
                    ...
                    if (StringUtil.isNotEmpty(attributes[0])
                        ...
                        this.processErrorsForUpload(resultPostByTitle, uploadErrors, lineNumber, attributes);
                    }
                    ...
                    this.processErrorsForUpload(null, uploadErrors, lineNumber, attributes);
                }
    ...
    /**
     * <h2>Errors For Upload Method</h2>
     * <p>
     * Errors For CSV File Upload Method
     * </p>
     * 
     * @param resultPotByTitle, uploadErrors, lineNumber, attributes
     */
    private void processErrorsForUpload(Post resultPostByTitle, List<String> uploadErrors, int lineNumber,
            String[] attributes) {
        if (resultPostByTitle != null) {
            uploadErrors.remove(0);
            uploadErrors.add("Post Title Data always Exist! Error At Row " + lineNumber);
        }
        if (StringUtils.isEmpty(attributes[0]) || attributes.length == 0) {
            uploadErrors.remove(0);
